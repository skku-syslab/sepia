...
max_num=0
for dir in one_flow_* two_flow_* three_flow_* four_flow_* five_flow_* six_flow_* seven_flow_* eight_flow_*; do
    if [ -d "$dir" ]; then
...
for ((iteration=0; iteration<REPEAT_COUNT; iteration++)); do
    ...
    # 1 flow test
    echo ""
    echo "[1/8] Running one flow test..."
    ./one_flow_test.sh
    if [ $? -eq 0 ]; then
    ...
    # 2 flow test
    echo ""
    echo "[2/8] Running two flow test..."
    ./two_flow_test.sh
    if [ $? -eq 0 ]; then
    ...
    # 3 flow test
    echo ""
    echo "[3/8] Running three flow test..."
    ./three_flow_test.sh
    if [ $? -eq 0 ]; then
    ...
    # 4 flow test
    echo ""
    echo "[4/8] Running four flow test..."
    ./four_flow_test.sh
    if [ $? -eq 0 ]; then
    ...
    # 5 flow test
    echo ""
    echo "[5/8] Running five flow test..."
    ./five_flow_test.sh
    if [ $? -eq 0 ]; then
    ...
    # 6 flow test
    echo ""
    echo "[6/8] Running six flow test..."
    ./six_flow_test.sh
    if [ $? -eq 0 ]; then
    ...
    # 7 flow test
    echo ""
    echo "[7/8] Running eight flow test..."
    ./seven_flow_test.sh
    if [ $? -eq 0 ]; then
        echo "✓ Seven flow test completed"
    else
        echo "✗ Seven flow test failed"
    fi
    sleep 3

    # 8 flow test
    echo ""
    echo "[8/8] Running eight flow test..."
    ./eight_flow_test.sh
    if [ $? -eq 0 ]; then
