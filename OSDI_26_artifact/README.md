...
(If you skipped the toy experiments) Note: Before running the artifact scripts, configure your environment-specific values in
...
- Before running Table 1, build the DRAM-traffic measurement tool (used by `four_flow_test.sh`):
  ```bash
  cd /usr/src/sepia/OSDI_26_artifact/table1
  ./tools/prepare_table1_tools.sh
  ```

- Note: perf event names are microarchitecture-dependent. If `measure_dram_traffic.bin` fails to start (event encoding/open fails), update `tools/measure_dram_traffic.cpp` based on your CPU:
  - `const int num_imc = 12;` (number of IMCs)
  - `icx_unc_imc%d::UNC_M_CAS_COUNT` (IMC PMU/event name; ALL/RD/WR)
