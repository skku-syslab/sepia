...
# Create per-group page-index sequences for the Tetris pattern.
#
# This runs `py_scripts/create_page_index_sequence.py` for every page group in
# parallel and waits for all jobs to finish.
...
for page_index in $(seq $start 1 $end); do
    python3 py_scripts/create_page_index_sequence.py $page_index &
done
