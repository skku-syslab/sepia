"""LLC slice-hash helper used by Figure 10 scripts.

This module computes LLC slice numbers from (physical) addresses using:
- `common/base_sequence.pkl`: base permutation sequence
- `common/hash_mask.h`: a list of bit masks (hex literals)

The hash model:
1) Computes an index from address bits above the 64B offset.
2) Computes parity bits from (address & mask) for each mask.
3) XORs the index with the parity-derived permutation bits.
4) Maps the permuted index through BASE_SEQUENCE to obtain a slice id.
"""
...
def load_masks_from_header(path="common/hash_mask.h"):
    # `hash_mask.h` is intentionally parsed as plain text.
...
def popcount_u64(x: np.ndarray) -> np.ndarray:
    # Vectorized popcount parity; falls back for older NumPy.
...
def build_perm_bits(addresses_u64: np.ndarray) -> np.ndarray:
    # Build parity bits for each mask and pack them into an integer.
