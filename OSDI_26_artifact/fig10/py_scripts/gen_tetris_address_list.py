"""Generate a Tetris-style address list from precomputed page sequences.

Usage:
    python3 py_scripts/gen_tetris_address_list.py <workingset_size_kb>

This script reads per-group page-index sequences produced by
`create_page_index_sequence.py`, then interleaves them across page groups to
emit a cache-line-granular address list.

Output:
- data/address_list-tetris/address_list.<workingset_size_kb>.csv
"""
...
def generate(output_file):
    # Number of cache lines to emit for the requested working set.
    want_num_cachelines = workingset_size_byte >> 6
    curr_num_cachelines = 0
    page_group_index = 0

    while True:
        # Round-robin over groups; within each group, advance the sequence index.
        ...
        # Emit all cache lines in the selected 4KiB page.
...
output_file_name = f"data/address_list-tetris/address_list.{workingset_size_kb}.csv"
